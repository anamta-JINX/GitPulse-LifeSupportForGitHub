...
## 1.6.2 — GitHub Ready (2026-09-11)

- Rebuilt the README as a product-level GitHub landing page using real GitPulse UI captures.
- Added a green GitPulse banner, real dashboard screenshot, Pulse Span screenshot, workflow visual and UI demo GIF.
- Added professional project structure, clone/run/build/test documentation and GitHub Actions notes.
- Added contribution and security guidance.
- Updated project ownership and licensing to Anamta Gohar, all rights reserved.
- Bumped the application and worker protocol version to 1.6.2.


## 1.6.1 — Circular Override (2026-09-08)

- Restored the circular pulses-today dashboard as the primary repository visual.
- Polished the original ring with a subtle double-track and keep its center count tied to the real scheduled target (including 0/0 when no span is set).
- Increased dark/light separation and green accent contrast for a cleaner, sleeker UI.
- Kept primary actions minimal: Pulse now, Pulse span, and a compact More menu.
- Made Pulse now a true manual override: it always creates one pulse even after
  today's scheduled quota is complete. Scheduled automation still obeys its quota.
- Track actual pulse count separately from scheduled slots so manual over-quota
  pulses remain visible (for example 6/5) without corrupting the automatic plan.
- Retained the PyInstaller .spec build fix and fresh isolated Windows build flow.


## 1.6.0 — Unified Span (2026-09-08)

- Fixed stale preview saves: Save reads and validates current form values, even
  before the live preview debounce finishes. Invalid input cannot save an old plan.
- Removed daily count/time inputs from repository details. Pulse span is now the
  only scheduling editor; editing a connection preserves its complete schedule.
- Persisted start/end window edits alongside the generated dates and pulse slots.
- Reopen the full saved span without silently changing its start, days or total.
- Added live preview and one Save span action; removed Generate and Clear plan.
- Support 1–365 days with an exact total, random daily counts and at least one
  pulse on every date. Reject impossible budgets with inline explanations.
- Preserve past slots and completed-date progress when editing active spans;
  protect today's already-completed pulse count.
- Prevent hidden daily defaults from invalidating or overriding a saved span.
  New repositories have no schedule until a span is saved.
- Fix next-pulse status after today's target is complete: show the next planned
  date rather than reporting the entire span complete.
- Simplify the dashboard: neutral dark colors, compact progress cards, one
  automation control and a More menu for secondary actions. Remove duplicate
  completion controls and manual Refresh.
- Prevent oversized entry fields from forcing wide form columns; preserve
  calendar scroll position when selecting a date.
- Keep existing pause and Windows-startup preferences when editing spans.
- Add behavioral regression tests for span editing and the actual Save handlers.
