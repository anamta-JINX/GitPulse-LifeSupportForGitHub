...
class GitService:
    ...
    def _prepare_cache_for_pulse(self, repo_dir: Path) -> None:
        ...
        if unexpected:
            raise RuntimeError(
                ...
                "No commit was made. Remove the affected cache under ~/.gitpulse/repos and retry."
            )
    ...
    def pulse(self, repo: RepoConfig, push: bool = True, force: bool = False) -> PulseResult:
        ...
        # Pulse Now is an explicit manual override: it must create exactly one
        # commit even when the scheduled quota is already complete (or today
        # has no scheduled span entry). Keep the configured target visible
        # when one exists; use 1 only to avoid an awkward 1/0 line.
        commit_target = target if target > 0 else 1
        result = self._commit_one(repo, repo_dir, branch, current + 1, commit_target)
        if push:
            self.run_git(["push", "origin", branch], cwd=repo_dir, timeout=240)
        return result
