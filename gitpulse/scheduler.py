...
from .utils import format_hhmm, format_hhmm_12, parse_hhmm, parse_repo_url, planned_times_for, repo_commit_url, today_key

LOCK_DIR = APP_DIR / "locks"
CREATE_NO_WINDOW = 0x08000000 if os.name == "nt" else 0
LOCAL_SYNC_INTERVAL_SECONDS = 3600
LOCAL_SYNC_RETRY_SECONDS = 300
WORKER_PROTOCOL = "1.6.2-circular-override-worker-1"
...
def validate_repo(repo: RepoConfig) -> list[str]:
    ...
    # A saved span is authoritative. Legacy daily settings must never block it.
    if repo.calendar_plan or repo.schedule_mode == "span":
        for date_key, values in repo.calendar_plan.items():
            try:
                datetime.strptime(date_key, "%Y-%m-%d")
                minutes = [parse_hhmm(value) for value in values]
                if not 1 <= len(minutes) <= 100 or len(set(minutes)) != len(minutes):
                    raise ValueError("Each planned day needs 1–100 unique pulse times.")
            except (TypeError, ValueError) as exc:
                errors.append(f"Invalid span on {date_key}: {exc}")
        return errors
...
def _random_daily_counts(
    ...
    daily_capacity: int,
    variant: int = 0,
    minimum_counts: list[int] | None = None,
) -> list[int]:
    ...
    rng = random.Random(seed)
    minimums = minimum_counts or [1 for _ in range(days)]
    counts = list(minimums)
    remaining = total_pulses - sum(counts)
    if remaining < 0 or any(count > daily_capacity for count in counts):
        raise ValueError("The pulse total or time window is below the required daily minimums.")
    ...
    if days > 1 and len(set(counts)) == 1:
        source = next((index for index, value in enumerate(counts) if value > minimums[index]), None)
...
def generate_automatic_plan(
    ...
    """Distribute an exact budget across 1–365 days, at least one each day."""
    try:
    ...
    try:
        dates = [(first_day + timedelta(days=i)).isoformat() for i in range(day_count)]
    except OverflowError as exc:
        raise ValueError("The span ends beyond the supported calendar range.") from exc
    ...
    minimum_counts = [max(1, int((minimums or {}).get(key, 1))) for key in dates]
    if any(count > daily_capacity for count in minimum_counts):
        raise ValueError("This time window is too small for pulses already completed today.")
...
def _sync_span_state(state: dict, config: AppConfig) -> dict:
    ...
    for repo in config.repositories:
        ...
        if existing.get("fingerprint") != fingerprint:
            previous_times = existing.get("daily_times", {})
            completed_dates = [
                key for key in existing.get("completed_dates", [])
                if key in dates and (
                    previous_times.get(key) == repo.calendar_plan[key]
                    or (not previous_times and key < today_key())
                )
            ]
            existing = {
        ...
        existing["daily_times"] = {key: list(repo.calendar_plan[key]) for key in dates}
    ...
    for repo in config.repositories:
        ...
        if existing.get("fingerprint") != fingerprint:
            ...
            existing = {
                ...
                "actual_count": max(int(existing.get("actual_count", 0) or 0), completed),
                "last_error": "",
            ...
            existing.setdefault("actual_count", len(existing.get("done", [])))
...
def mark_progress(repo_id: str, completed_count: int, error: str = "") -> None:
    ...
    times = item.get("times", [])
    raw_count = max(0, int(completed_count))
    count = min(raw_count, len(times))
    item["done"] = [str(index) for index in range(1, count + 1)]
    item["actual_count"] = max(int(item.get("actual_count", 0) or 0), raw_count)
...
def run_one_pulse(repo: RepoConfig, mode: str = "Manual", force: bool = False) -> PulseResult:
    ...
    with repo_lock(repo.id):
        try:
            result = service.pulse(repo, push=True, force=force)
