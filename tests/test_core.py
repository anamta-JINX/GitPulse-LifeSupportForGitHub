...
from gitpulse.scheduler import generate_automatic_plan, local_sync_is_due, schedule_for_day, validate_local_sync, validate_repo
...
class GitPulseCoreTests(unittest.TestCase):
    def test_version_is_1_6_2(self) -> None:
        self.assertEqual(__version__, "1.6.2")

    def test_windows_builder_does_not_mix_paths_with_spec(self) -> None:
        builder = (Path(__file__).resolve().parent.parent / "scripts" / "windows" / "build_exe.bat").read_text(encoding="utf-8")
        build_lines = [line.strip().lower() for line in builder.splitlines() if "pyinstaller" in line.lower() and "packaging\\gitpulse.spec" in line.lower()]
        self.assertEqual(len(build_lines), 1)
        self.assertNotIn("--paths", build_lines[0])
        self.assertIn("--clean", build_lines[0])
    ...
    def test_span_calendar_is_scrollable_and_has_no_per_day_input(self) -> None:
        ...
        self.assertIn('"Total pulses"', planner)
        ...
        self.assertIn('"Save span"', planner)
    ...
    def test_pulse_now_is_a_true_quota_override(self) -> None:
        source = (Path(__file__).resolve().parent.parent / "gitpulse" / "ui.py").read_text(encoding="utf-8")
        self.assertIn('run_one_pulse(repo, mode="Pulse now", force=True)', source)
        self.assertNotIn('self._pulse_busy_repo_id or done >= target', source)
        self.assertIn('ProgressRing(body, done, target', source)

    def test_git_service_force_pulse_creates_after_target(self) -> None:
        service = GitService()
        repo = RepoConfig(
            id="force-pulse",
            name="Force",
            repo_url="https://github.com/example/force",
            commit_email="dev@example.com",
            commits_per_day=5,
        )
        repo_dir = Path("C:/gitpulse-force-test")
        forced_result = Mock(created=True, count=6, target=5, commit_hash="abc123", branch="main")
        with (
            patch.object(service, "ensure_repo", return_value=(repo_dir, "main")),
            patch.object(service, "read_daily_count", return_value=5),
            patch.object(service, "_commit_one", return_value=forced_result) as commit_one,
            patch("gitpulse.git_service.pulse_target_for_date", return_value=5),
        ):
            result = service.pulse(repo, push=False, force=True)
        self.assertIs(result, forced_result)
        commit_one.assert_called_once_with(repo, repo_dir, "main", 6, 5)
